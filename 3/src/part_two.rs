...
fn main() {
    ...
    let mut potential_gears : Vec<Position> = vec![];
    ...
    for (y, line) in lines.enumerate(){
        for (x, char) in line.chars().enumerate(){
            ...
            if char == '*'{
                potential_gears.push(Position{x,y});
            }
    ...
    for potential_gear in potential_gears{
        ...
        let adjacent = potential_gear.generate_adjacent();
        ...
        if found.len() == 2{
            sum += found.get(0).unwrap() * found.get(1).unwrap();
        }
    }
    println!("{}", sum);
}
